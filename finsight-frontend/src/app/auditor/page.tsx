...
export default function AuditorPage() {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-card/90 backdrop-blur-md border border-primary/20 p-2 text-xs rounded shadow-lg">
  ...
  if (isLoading || !report) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-muted-foreground animate-pulse">Compiling Ledger Data for GBM Report...</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-10 px-4 max-w-5xl animate-fade-in print:py-0 print:m-0 print:max-w-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 print:hidden">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2 shadow-lg shadow-primary/20"
        >
      ...
      {/* Report Container */}
      <div ref={reportRef} className="bg-card rounded-2xl border border-primary/10 shadow-xl overflow-hidden print:shadow-none print:border-none print:bg-white print:text-black">
         {/* Report Header */}
         <div className="p-8 border-b border-primary/10 bg-muted/30 print:bg-gray-100 flex justify-between items-end">
            <div>
               <h2 className="text-3xl font-extrabold tracking-tight mb-2">General Body Meeting Report</h2>
               <p className="text-muted-foreground print:text-gray-600 flex items-center gap-2">
                  <CalendarDays className="h-4 w-4" /> Statement Period: {new Date().toLocaleDateString('default', { month: 'long', year: 'numeric'})}
               </p>
            </div>
            <div className="text-right">
               <p className="text-xs uppercase tracking-widest text-muted-foreground print:text-gray-500 mb-1">Generated By</p>
               <p className="font-semibold text-primary print:text-black flex items-center gap-1.5 justify-end">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-8 bg-card print:bg-white print:gap-4 print:p-4">
            <div className="p-6 rounded-xl border bg-muted/10 print:bg-gray-50 print:border-gray-200">
               <p className="text-sm font-medium text-muted-foreground print:text-gray-500 mb-2 flex items-center gap-2">
                  <TrendingUp className="h-4 w-4 text-emerald-500" /> Total Inflow
               </p>
               <p className="text-3xl font-bold font-mono text-emerald-500">{formatCurrency(report.totalIncome, currency)}</p>
            </div>
            <div className="p-6 rounded-xl border bg-muted/10 print:bg-gray-50 print:border-gray-200">
               <p className="text-sm font-medium text-muted-foreground print:text-gray-500 mb-2 flex items-center gap-2">
         ...
         <div className="px-8 pb-8 grid grid-cols-1 md:grid-cols-2 gap-8 print:px-4 print:pb-4">
            ...
            <div className="space-y-8">
               <div>
                  <h3 className="text-lg font-bold mb-4 border-b border-primary/10 pb-2 print:border-gray-200">Highest Expenditures (Vendors)</h3>
            ...
            <div className="space-y-8">
               <div>
                  <h3 className="text-lg font-bold mb-4 border-b border-primary/10 pb-2 print:border-gray-200">AI Financial Projections</h3>
                  <div className="space-y-4 p-4 rounded-lg bg-primary/5 border border-primary/10 print:bg-gray-50 print:border-gray-200">
                     <div className="flex justify-between items-center">
                        <span className="text-sm text-muted-foreground print:text-gray-600">Avg. Daily Burn Rate</span>
                        <span className="font-mono font-bold text-destructive">{formatCurrency(report.burnRate, currency)}/day</span>
                     </div>
                     <div className="flex justify-between items-center">
                        <span className="text-sm text-muted-foreground print:text-gray-600">Projected Exp. (Next Month)</span>
                        <span className="font-mono font-bold text-amber-500">{formatCurrency(report.projectedExpense, currency)}</span>
                     </div>
                  </div>
               </div>

                <div>
                  <h3 className="text-lg font-bold mb-4 border-b border-primary/10 pb-2 print:border-gray-200">Category Breakdown</h3>
                  <div className="space-y-3">
                     {report.categoryBreakdown.slice(0, 5).map((cat, idx) => (
                        <div key={idx} className="flex justify-between items-center text-sm">
                           <span className="text-muted-foreground print:text-gray-700">{cat.categoryName}</span>
         ...
         <div className="p-4 text-center text-xs text-muted-foreground border-t border-primary/5 bg-muted/10 print:bg-white print:border-gray-200 print:text-gray-400">
