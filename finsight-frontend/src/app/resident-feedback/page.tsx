...
export default function ResidentFeedbackDashboard() {
  ...
  return (
    <div className="container mx-auto py-10 px-4 max-w-7xl print:bg-white print:p-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 print:hidden">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Management, Insights & Community Satisfaction</p>
        </div>
      ...
      <div className="hidden print:block mb-8 border-b pb-4">
        ...
        <p className="text-muted-foreground">{survey?.quarter} {survey?.year} - FinSight Operational Audit</p>
      </div>

      {activeTab === "insights" && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          {!survey ? (
             <div className="text-center py-20 border-2 border-dashed rounded-3xl bg-muted/20 print:hidden">
                <AlertCircle className="h-12 w-12 text-muted-foreground mx-auto mb-4" />
                <h2 className="text-2xl font-bold">No Active Survey</h2>
                <p className="text-muted-foreground mt-2 mb-6">Create a new survey in the Management tab to start collecting feedback.</p>
                <button 
                  onClick={() => setActiveTab("management")}
                  className="px-6 py-2.5 bg-primary text-primary-foreground rounded-xl font-bold shadow-lg shadow-primary/20"
                >
                  Create Survey Now
                </button>
             </div>
            ...
            <>
              <div className="flex justify-between items-center mb-10 bg-primary/5 p-6 rounded-2xl border border-primary/10 print:bg-white print:border-none print:px-0">
                <div>
                   <h2 className="text-xl font-bold">Audit Period: {survey.quarter} {survey.year}</h2>
                   <p className="text-sm text-muted-foreground">Generated by Gemini AI on {new Date().toLocaleDateString()}</p>
                </div>
                <div className="flex gap-3 print:hidden">
                  ...
                  <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-xl font-medium shadow-lg shadow-primary/20">
              ...
              {data?.executiveSummary && (
                <div className="mb-10 p-8 rounded-3xl border-2 border-primary/20 bg-primary/5 relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:block">
                <div className="p-8 rounded-3xl border bg-card shadow-lg relative overflow-hidden print:mb-8 print:shadow-none">
                ...
                <div className="p-6 rounded-3xl border bg-indigo-500/5 border-indigo-500/10 shadow-lg relative h-full print:shadow-none print:border-indigo-100">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-bold flex items-center gap-2">
                       <CheckCircle2 className="h-5 w-5 text-indigo-500" /> Operational Action Items
                    </h2>
                    <span className="text-[10px] font-bold bg-indigo-500/20 text-indigo-600 px-2 py-1 rounded-full uppercase print:hidden">Gemini Insight Engine</span>
                  </div>
                  <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 scrollbar-hide print:max-h-none print:overflow-visible">
                    {data?.insights.map((insight, idx) => (
                      <div key={idx} className="p-5 rounded-2xl bg-background/50 border hover:border-indigo-500/30 transition-all group print:mb-4 print:break-inside-avoid">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-xs font-mono font-bold text-muted-foreground">{(insight.sentimentScore * 100).toFixed(0)}% Mood Score</span>
                        </div>
                        <p className="text-sm text-muted-foreground mb-3 leading-relaxed">{insight.aiSummary}</p>
                        <div className="p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/10 text-sm text-indigo-700 font-medium">
                          <div className="flex items-center gap-2 mb-2 text-[10px] font-bold uppercase tracking-wider text-indigo-400">
      ...
      {activeTab === "management" && (
        <div className="max-w-2xl mx-auto animate-in slide-in-from-bottom-4 duration-500">
           <div className="p-8 rounded-3xl border bg-card shadow-xl">
             <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
               <QrCode className="h-6 w-6 text-primary" /> Create Quarterly Survey
             </h2>
             <p className="text-muted-foreground mb-8">This will automatically generate a new Google Form and archive any existing active survey for this association.</p>
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-2">
                   ...
                   <select 
                     ...
                     className="w-full p-3 rounded-xl bg-muted border-none focus:ring-2 ring-primary/20 transition-all font-medium"
                   >
                     <option>Q1</option>
                     <option>Q2</option>
                     <option>Q3</option>
      ...
      {activeTab === "bug" && (
        <div className="max-w-2xl mx-auto animate-in slide-in-from-bottom-4 duration-500">
           <div className="p-8 rounded-3xl border bg-card shadow-xl">
             ...
             <p className="text-muted-foreground mb-8">Submit bug reports or suggest new features for the FinSight platform. Your feedback is stored directly and reviewed by the audit team.</p>
             ...
             <form onSubmit={handleSubmitFeedback} className="space-y-6">
               <div className="space-y-2">
                 ...
                 <input 
                   ...
                   className="w-full p-3 rounded-xl bg-muted border-none focus:ring-2 ring-primary/20 transition-all"
                 />
               </div>
                   ...
                   {["BUG", "FEATURE", "GENERAL"].map((t) => (
                     <button
                       ...
                       className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all border ${
                         feedbackType === t ? "bg-primary border-primary text-primary-foreground" : "bg-muted border-transparent text-muted-foreground"
                       }`}
                     >
                       {t}
                     </button>
               ...
               <div className="space-y-2">
                 ...
                 <textarea 
                   ...
                   className="w-full p-4 rounded-xl bg-muted border-none focus:ring-2 ring-primary/20 transition-all resize-none"
                 />
               </div>
